...
```{r plotly_total_cases, echo=FALSE, fig.width=10, fig.height=6}
...
for (k in 2:length(top_10_countries)) {
  ...
  fig <- add_trace(fig, y=~y, x=~x, data=country_df, type="scatter", mode="lines", name = colnames(top10_df_wide_count)[k])
}

fig <- fig %>% layout(
    title = "Total Confirmed Cases Since First 25 Cases",
    xaxis = list(title='Days Since First 25 Cases', rangeslider = list(type = "int")),
...
for (k in 2:length(top_10_countries)) {
  ...
  fig <- add_trace(fig, y=~y, x=~x, data=country_df, type="scatter", mode="lines", name = colnames(top10_df_wide_daily)[k])
}
