...
top_10_countries <- (head(country_table$Country_Region, 16))
...
ggplot(top10_df, aes(x = `Days Since First Case`, y=Count, group=Country_Region)) +
  ...
  ggtitle("Total Confirmed Cases Since First 25 Cases") +
  xlab(label="Days Since First 25 Cases") +
...
ggplot(top10_df, aes(x = `Days Since First Case`, y=Daily_Count, group=Country_Region)) +
  geom_line(aes(color = Country_Region), size=0.75) +
  ggtitle("Daily Confirmed Cases Since First 25 Case") +
  xlab(label="Days Since First 25 Cases") +
  ylab(label="Daily Count") +
  theme_light() +
  theme(legend.position="bottom", legend.title = element_blank())
```
